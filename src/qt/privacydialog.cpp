                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zJIYO ought to be enough for anybody." - Bill Gates, 2017
    ui->zJIYOpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzJIYOSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzJIYO->setEnabled(false);
        ui->pushButtonMintzJIYO->setToolTip(tr("zJIYO is currently disabled due to maintenance."));

        ui->pushButtonSpendzJIYO->setEnabled(false);
        ui->pushButtonSpendzJIYO->setToolTip(tr("zJIYO is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zJIYOpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzJIYO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zJIYO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zJIYO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zJIYO in ") +
...
void PrivacyDialog::on_pushButtonSpendzJIYO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zJIYO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zJIYO
        sendzJIYO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zJIYO
    sendzJIYO();
}

void PrivacyDialog::on_pushButtonZJiyoControl_clicked()
{
    ZJiyoControlDialog* zJiyoControl = new ZJiyoControlDialog(this);
    zJiyoControl->setModel(walletModel);
    zJiyoControl->exec();
}

void PrivacyDialog::setZJiyoControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzJiyoSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzJIYO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Jiyo Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zJIYOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zJIYO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " JIYO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zJIYO</b>";
    ...
    // use mints from zJiyo selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZJiyoControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZJiyoControlDialog::GetSelectedMints();
    }
    ...
    // Clear zjiyo selector in case it was used
    ZJiyoControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zJiyo Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Jiyo, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zJiyo Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zJIYO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zJIYO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zJIYO "));
    ui->labelzJIYOAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzJIYOSyncStatus->setVisible(fShow);
}
