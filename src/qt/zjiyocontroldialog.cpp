...
#include "ui_zjiyocontroldialog.h"
...
std::list<std::string> ZJiyoControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZJiyoControlDialog::listMints;

ZJiyoControlDialog::ZJiyoControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZJiyoControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZJiyoControlDialog::~ZJiyoControlDialog()
{
    delete ui;
}

void ZJiyoControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZJiyoControlDialog::updateList()
{
...
void ZJiyoControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZJiyoControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZJiyo_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZJiyoControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZJiyoControlDialog::GetSelectedMints()
{
...
void ZJiyoControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
