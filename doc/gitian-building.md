...
*Setup instructions for a gitian build of Jiyo using a Debian VM or physical system.*
...
Clone the git repositories for jiyo and gitian and then checkout the jiyo version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the jiyo repository
...
To build Jiyo (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the jiyo repository.
...
```bash
    ...
    From https://github.com/eastcoastcrypto/Jiyo
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/Jiyo.git
